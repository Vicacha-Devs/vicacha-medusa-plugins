...
export type EditMediaFormResult = {
...
type EditMediaFormProps = {
  images?: { id: string; url: string }[]
  thumbnail?: string | null
  onSave: (result: EditMediaFormResult) => Promise<void>
  isPending?: boolean
}

type Media = z.infer<typeof MediaSchema>

export const EditMediaForm = ({ images, thumbnail, onSave, isPending = false }: EditMediaFormProps) => {
  ...
  const form = useForm<EditMediaSchemaType>({
    defaultValues: {
      media: getDefaultValues(images, thumbnail),
    },
  ...
  const handleSubmit = form.handleSubmit(async ({ media }) => {
    ...
    const resolvedThumbnail = withUpdatedUrls.find((m: Media) => m.isThumbnail)?.url ?? null

    try {
      await onSave({
        images: withUpdatedUrls.map((m: Media) => ({ url: m.url, id: m.id })),
...
const getDefaultValues = (
  images: { id: string; url: string }[] | null | undefined,
